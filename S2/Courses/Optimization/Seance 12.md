...
## 2. Exemple Classique : Problème du Régime Alimentaire

### 2.1 Contexte

Une personne souhaite composer un régime alimentaire quotidien qui couvre ses besoins nutritionnels tout en minimisant le coût total des aliments consommés.

### 2.2 Données

Elle dispose de 6 aliments, chaque aliment apporte une certaine quantité de protéines, de vitamines et de calories. Le régime doit respecter des apports nutritionnels minimaux.
...
### 2.3 Besoins Nutritionnels

- **Énergie :** 2000 kcal
- **Protéines :** 55 g
- **Calcium :** 800 mg

### 2.4 Limites Journalières
...
$$\min f = 3x_1 + 24x_2 + 13x_3 + 9x_4 + 20x_5 + 19x_6$$
...
- En dimension deux, l'ensemble admissible est généralement représenté par un **polygone convexe**, éventuellement non borné
- Les coins de ce polygone sont appelés **sommets** ou **points extrêmes**
...
### 4.2 Étapes de la Méthode Graphique
...
### 4.3 Cas Possibles pour le Polygone de Contraintes
...
### 5.1 Problème 1 : Solution Unique
...
### 5.2 Problème 2 : Pas de Solution (Non Faisable)
...
### 5.3 Problème 3 : Problème Non Borné

$$\begin{cases} \max z = x_1 - x_2 \\ -2x_1 + x_2 \leq -1 \\ -x_1 - 2x_2 \leq -2 \\ x_1, x_2 \geq 0 \end{cases}$$

**Résolution :**
- Première contrainte : $-2x_1 + x_2 \leq -1 \Rightarrow x_2 \leq 2x_1 - 1$
- Deuxième contrainte : $-x_1 - 2x_2 \leq -2 \Rightarrow x_1 + 2x_2 \geq 2$
- Point admissible : (1,1) est admissible
...
### 5.4 Problème 4 : Solutions Multiples
...
### 5.5 Problème 5 : Maximisation avec Deux Contraintes
...
### 5.6 Problème 6 : Maximisation du Profit
...
### 5.7 Problème 7 : Maximisation avec Trois Contraintes
...
- **Solution optimale :** (0.00, 1.00) - Z = 2.00

### 5.8 Problème 8 : Région Admissible Vide
...
- Si $x_1 > 3$ et $x_2 \geq 0$, alors $3x_1 + 2x_2 \geq 9$, ce qui viole $3x_1 + 2x_2 \leq 6$
- **La région admissible est vide**
...
### 6.2 Concept Fondamental : Exploration des Sommets

L'algorithme du simplexe exploite la propriété fondamentale de la programmation linéaire : si une solution optimale existe, elle se trouve toujours à un sommet (ou point extrême) du polytope des contraintes.

**Principe :** L'algorithme du simplexe se déplace itérativement d'un sommet admissible à un sommet admissible voisin, en améliorant la valeur de la fonction objective à chaque étape. Il s'arrête lorsqu'il atteint un sommet où aucune amélioration n'est possible, ce qui correspond à la solution optimale.

### 6.3 Mise sous Forme Standard
...
3. **Transformation des contraintes d'inégalité en égalités :**
   - Pour $\sum a_{ij}x_j \leq b_i$ : ajouter une variable d'écart $s_i \geq 0$
     $$\sum a_{ij}x_j + s_i = b_i$$
   - Pour $\sum a_{ij}x_j \geq b_i$ : soustraire une variable d'excédent $e_i \geq 0$
     $$\sum a_{ij}x_j - e_i = b_i$$

4. **Variables non négatives :** $x_j \geq 0, s_i \geq 0, e_i \geq 0$
   - Si $x_j \leq 0$ : remplacer par $x_j^+ = -x_j$
   - Si $x_j \in \mathbb{R}$ : remplacer par $x_j = x_j^+ - x_j^-$ avec $x_j^+, x_j^- \geq 0$

#### Exemple 1 : Transformation Standard
...
2. Transformation en égalités :
   - $4y_1 - 2y_2 + y_3 - y_4 + s_1 = 65$
   - $y_1 + y_2 + y_4 - y_3 - e_1 = 5$
   - $y_1 + y_2 = 10$
...
#### Exemple 2 : Transformation Standard
...
**Remarque :** Un coefficient négatif dans la ligne de Z indique qu'une amélioration est encore possible.

### 6.5 Algorithme du Simplexe (Cas de Maximisation)
...
### 6.8 Choix de la Variable Sortante
...
## 7. Exemple Détaillé de Résolution par le Simplexe
...
- $x_1 = 0$, $x_2 = 600$, $x_3 = 600$, $x_4 = 400$, $x_5 = 0$
- $Z = 50 \times 600 = 30000$
...
- $x_3 = 0$ : La première contrainte est saturée (entièrement utilisée)
- $x_4 = 200$ : La deuxième contrainte a une marge de 200 unités non utilisées
- $x_5 = 0$ : La troisième contrainte est saturée (entièrement utilisée)

---

## 8. Cas Particuliers du Simplexe

### 8.1 Solutions Multiples
...
- La variable entrante peut augmenter indéfiniment sans violer les contraintes
...
Si une variable de base a une valeur $b_i$ nulle, le test du ratio minimum peut conduire à un ratio de zéro, ce qui peut potentiellement entraîner des cycles :
- La variable sortante quitte la base avec une valeur déjà nulle
- La base change, mais la solution reste exactement la même
- La valeur de Z ne s'améliore pas

### 8.4 Absence de Solution Réalisable
...
### 8.5 Solution de Base Réalisable Initiale
...
## 9. Variantes du Simplexe

### 9.1 Méthode du Simplexe Dual
### 9.2 Méthode du Simplexe Borné (Bounded Variable Simplex)
### 9.3 Méthode du Simplexe Révisé (Revised Simplex Method)

---

## 10. Autres Algorithmes de Programmation Linéaire

### 10.1 Algorithmes Spécialisés

- **Algorithme de Transport** : Distribution de biens de sources à destinations, minimisation des coûts logistiques
- **Algorithme d'Affectation (Méthode Hongroise)** : Affectation optimale de ressources à des tâches
...
- **Décomposition de Dantzig-Wolfe** : Problèmes de grande taille avec structure bloc-diagonale, planification multi-étapes

### 10.2 Logiciels de Programmation Linéaire
...
## 11. Avantages et Limites de la Méthode du Simplexe

### 11.1 Avantages

- Très performante pour la plupart des problèmes réels (souvent converge en O(n) à O(n³) itérations)
...
- Interprétation économique : Les variables duales donnent des insights utiles (valeur marginale des ressources, sensibilité aux contraintes)

### 11.2 Limites

- Dans le pire cas, complexité théorique exponentielle : O(2^n) itérations
- Sensibilité aux dégénérescences : Risque de cyclage (boucles infinies) si les règles anti-cyclage ne sont pas appliquées
